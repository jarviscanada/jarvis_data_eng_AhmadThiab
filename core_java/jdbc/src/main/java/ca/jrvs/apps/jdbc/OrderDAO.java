...
public class OrderDAO extends DataAccessObject<Order> {
  ...
  private final static String GET_BY_ID = "SELECT c.first_name, c.last_name, c.email, o.order_id, o.order_id, o.creation_date, " +
      "o.total_due, o.status, s.first_name, s.last_name, s.email, ol.quantity, p.code, p,name, p.size, p.variety, p.price FROM orders o " +
      "JOIN customer c on o.customer_id=c.customer_id JOIN salesperson s on o.salesperson_id=s.salesperson_id JOIN order_item ol " +
      "on ol.order_id=o.order_id JOIN product p on ol.product_id = p.product_id where o.order_id = ?";
    ...
    try(PreparedStatement statement = this.connection.prepareStatement(GET_BY_ID);){
      ...
      while(resultSet.next()){
        if(orderId==0){
          ...
          order.setSalespersonFirstName(resultSet.getString(9));
